...
void NeedlemanWunschForTranscript::initialize1(){
    for( int i=0; i<17; i++){
        for( int j=0; j<17; j++){
...
void NeedlemanWunschForTranscript::initialize2(){
    this->_exon_subsitition_matrix=this->nucleotideCodeSubstitutionMatrix.get_exon_subsitition_matrix();
    this->_intron_subsitition_matrix=this->nucleotideCodeSubstitutionMatrix.get_intron_subsitition_matrix();
    this->_splice_sites_subsitition_matrix=this->nucleotideCodeSubstitutionMatrix.get_splice_sites_subsitition_matrix();
    this->_start_stop_codon_subsitition_matrix=this->nucleotideCodeSubstitutionMatrix.get_start_stop_codon_subsitition_matrix();
}
...
//            _similarity += 1;
  ...
  //          _signs_sst << "-";
            //++_gaps;
            --i;
        } else { // Going to S(i, j-1) //insertion
            _alignment_a_sst << '-';
            _alignment_b_sst << _dna_b[j - 1];
//            _signs_sst << "+";
            //++_gaps;
...
void NeedlemanWunschForTranscript::print_results()
{
  ...
  //      std::cout << _alignment_a[i];
    ...
    std::string elementString = elements.str();
...
//    std::cout << elements.str() << std::endl;
    ...
    std::size_t pos = 0;
    while (pos < _alignment_a.length()) {
        std::cout << _alignment_a.substr(pos, linewidth) << std::endl;
//        std::cout << _signs.substr(pos, linewidth) << std::endl;
        std::cout << _alignment_b.substr(pos, linewidth) << std::endl;
        std::cout << elementString.substr(pos, linewidth) << std::endl << std::endl;
        pos += linewidth;
    }
