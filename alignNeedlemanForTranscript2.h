...
class SpliceSitePosition{
    private:
        size_t _donorSpliceSitePosition;
        size_t _acceptorSpliceSitePosition;
    public:
        SpliceSitePosition(size_t donorsSpliceSitePosition, size_t acceptorSpliceSitePosition);
        size_t getDonorSpliceSitePosition();
        size_t getAcceptorSpliceSitePosition();
};
...
class NeedlemanWunschForTranscript {
    ...
    double _exon_subsitition_matrix[17][17];
    ...
    double _intron_subsitition_matrix[17][17];
    ...
    double _start_stop_codon_subsitition_matrix[17][17];
    ...
    double _splice_sites_subsitition_matrix[17][17];
    ...
    NeedlemanWunschForTranscript(std::string dna_a, std::string dna_b, int startCodonPosition, int stopCodonPosition, std::vector<SpliceSitePosition>& splitSitePositions);
    void initialize();
...
//    void populate_subs_matrix(std::string subs_matrix_file);
    void dna_align();
    void print_results();
    ELEMENTS checkElements( size_t position );
};
